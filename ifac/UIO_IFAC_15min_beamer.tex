...
% Speaker notes ---------------------------------------------------------------
% Keep this as \shownotesfalse for the PDF sent to the projector/audience.
% Change it to \shownotestrue and compile a second PDF for your laptop.
% That presenter PDF places each slide on the left and its private notes on
% the right; use it only on the presenter display in an extended desktop.
\newif\ifshownotes
\shownotesfalse
\ifshownotes
...
\begin{frame}[plain]
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.73\textwidth}
      \hspace{0.35cm}\begin{minipage}{0.95\linewidth}
        ...
        \vspace{0.28cm}
        \large \textbf{Core idea:} use a bounded unknown-input observer as a reliable
        \textcolor{ifacgreen}{teacher} for a neural adaptive observer.
      \end{minipage}
    \end{column}
  ...
  \end{columns}
  \speakernote{
    \textbf{Memory cue:} Problem $\rightarrow$ certified teacher $\rightarrow$ learned residual.\par\smallskip
    Good morning everyone. Today I will present our work on learning with unknown input observers for robust nonlinear estimation. The problem is simple to state but difficult in practice: our physical model is nonlinear, and part of the dynamics is unknown. In a vehicle, for example, these unknown dynamics can be tire-force residuals, modelling errors, or external disturbances.\par\smallskip
    Our main idea is to combine a certified observer with a learning-based observer. The first layer gives a bounded and physically meaningful estimate, even when the usual unknown-input observer condition is not exactly satisfied. The second layer uses this reliable estimate as a teacher and learns the state-dependent residual dynamics. I will first explain the observer construction, then the learning layer, and finally show the vehicle results.
  }
\end{frame}
...
\centering
\begin{tikzpicture}
  \node[block, text width=2.35cm] (plant) at (0,0)
    {Plant\\$\dot x=\varphi(x,u)+B\mu_x$};
  \node[block, text width=2.55cm] (uio) at (3.55,1.22)
    {Layer 1: generalized UIO\\$\hat x_{\rm UIO},\ \hat\mu_x$};
  \node[learnblock, text width=2.45cm] (model) at (7.18,1.22)
    {Residual learner\\$\mu_\theta(\hat x_{\rm NN},\mathrm{UIO},u)$};
  \node[learnblock, text width=2.55cm] (nao) at (5.10,-1.46)
  ...
  \draw[flow] (plant.east) to[out=24,in=180]
    node[signal, pos=.55, above] {$y,\,u$} (uio.west);
  \draw[learnflow] (uio.east) -- (model.west);
  \draw[learnflow] (model.south) to[out=-90,in=26]
    node[signal, pos=.56, right] {$\mu_\theta$} (nao.east);
  \draw[flow] (plant.south) |- 
    node[signal, pos=.73, below] {$y,\,u$} (nao.west);
\end{tikzpicture}
...
% -----------------------------------------------------------------------------
\begin{frame}{Layer 2: online adaptation loop}
\vspace{0.02cm}
\centering
...
\begin{columns}[T,onlytextwidth]
  ...
  \begin{column}{0.38\textwidth}
    \small
    \begin{block}{What the guarantee means}
      \scriptsize
      Let $\Delta_{x,k}=\mu_{x,k}-\mu_{\theta,k}$. The LMI-designed gain
      $L_{\rm NN}$ remains fixed while $\theta_k$ adapts online.
      \vspace{0.10cm}
      \[
      ...
      \]
      Learning is therefore directed at reducing $\Delta_x$ without removing the
      observer's robustness certificate.
    \end{block}
  \end{column}
\end{columns}
\speakernote{
  \textbf{Memory cue:} Learn the residual, keep output feedback, and bound the remaining mismatch.\par\smallskip
  The neural network receives the current neural-state estimate, the UIO state estimate, the UIO unknown-input estimate, and the known input. It produces $\mu_{\theta,k}$, an approximation of the unknown dynamics. This learned residual is injected into the nonlinear observer model, while the output-error feedback $L_{\rm NN}(y-C\hat x_{\rm NN})$ keeps the estimate anchored to the data.\par\smallskip
  The four loss terms promote reference tracking, measurement consistency, agreement with the certified UIO teacher, and residual matching to the teacher proxy. The $\mathcal L_2$ inequality on the right bounds neural-state error by the remaining approximation error $\Delta_x$ plus an initial-condition transient. Thus the learning layer remains connected to robustness analysis.
}
\end{frame}
