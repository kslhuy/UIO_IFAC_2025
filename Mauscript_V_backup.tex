...
This paper unfolds as follows: Section \ref{sec_related_work} reviews related work. Section \ref{sec_related_work} formulates the problem. Section \ref{sec_vehiclemodel} presents the system model and problem formulation. Section \ref{sec_Distributed Observer} details the distributed observer, Section \ref{sec_Evolution_trust} presents the trust framework, Section \ref{sec_Weigth_Design} designs the observer weights.Section \ref{sec_Prove} provides the stability analysis. Section \ref{sec_simulation} discusses simulations in different scenarios, also application of estimated state to controller, Section \ref{sec_Conclusion} concludes with future directions.
...
\begin{itemize}
    ...
    \begin{enumerate}
        \item \textit{Physical Connectivity-Based Methods}: These analyze physical signals, such as device fingerprints or signal characteristics, to verify authenticity and assign trust scores. For example, unique wireless communication fingerprints can distinguish legitimate vehicles from spoofed ones.
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \textit{Trust-Informed Control}: When trust scores are available, data from neighbors can be weighted in observers or controllers, reducing the influence of potentially malicious inputs while retaining useful information.
    \end{itemize}
    % Noting limitations
...
We consider $N$ connected vehicles, and
...
where
\begin{itemize}
    \item ${s}_{i}$ denotes the coordinates of longitudinal location of the vehicle position;
    \item $v_{i}$ and $a_{i}$ are the velocity and acceleration, respectively;
    \item $u_{i}$ is the acceleration control signal; 
    \item $\tau_i$ is the engine's constant time lag.
    \item  $\V = \{1, 2, \dots, N\}$ is the set of vehicle.
\end{itemize}
We discretize the dynamics described above using a sampling period of \( T_s \), resulting in the following third-order state-space model for vehicle \( i \) in a homogeneous platoon (where \( \tau_i = \tau \)). We define the state vector for each vehicle as $x^{(i)} = \begin{bmatrix} s_i & v_i & a_i \end{bmatrix}^{\top}$, allowing us to rewrite the model in \eqref{eq_bicycle} as follows:
\begin{equation}
\begin{cases}
    \begin{aligned}
...
Define the collective state as $x = \col{x^{(1)}, \cdots , x^{(N)}}$, the state-space equation of the platoon is 
\begin{equation}
    \begin{cases}
        \begin{aligned}
            x(t+1) & = A x(t) + B u(t),\\
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
And we assume that each vehicle can sent or receive the data from the proceeding or following vehicles. 
...
The distributed observer serves as an effective methods to estimate the entire state $x$ of the platoon. In this section, we will briefly revisit existing findings on the distributed observer for the platoon. Following this, the main concept of this paper will be introduced. 
...
where 
\begin{itemize}
    \item $\hat{x}_{0}^{(i)}(t)$ is the state of the local observer $\LO_{i}$, which aimes to track the states of the $i$th vehicle. It means that
    \begin{equation}
    ...
    \item $\hat{x}^{(j)}_i(t)$ is the state of the distributed observer $\DO_{i}^{(j)}$, which aimes to track the states of the platoon. It means that 
    \begin{equation}
...
\begin{equation}
    \LT_{i,l}= \iota_{i,l}^\text{time} \cdot \iota_{i,l}^\text{velocity} \cdot \iota_{i,l}^\text{distance} \cdot \iota_{i,l}^{\text{accel}} 
\end{equation}
where $*$ represents the different components of the local trust, which will be explained in the following sections.



\subsubsection{Velocity}
The host vehicle $i$ calculates the reference velocity $v_{\text{ref}}(t)$ based on the leader's velocity $v_{\text{L}}(t-b_{\text{L}})$, adjusted by the estimated acceleration $a_{\text{L}}(t-b_{\text{L}})$ and the elapsed time $t_{\text{elap}}$ since the last leader beacon:
\begin{equation}
...
\begin{equation}
    v_{l,\text{H}} = 
        \begin{cases} 
            \max\left(1 - \left|\dfrac{\hat{v}_{0}^{(l)} - \hat{v}_{0}^{(i)}}{\hat{v}_{0}^{(j)}}\right|,  0\right), & \text{if} \  \hat{v}_{0}^{(i)} > 0, \\
            \max\left(1 - |\hat{v}_{0}^{(i)}|,  0\right), & \text{otherwise},
        \end{cases}
\end{equation}
...
\begin{equation}
    \iota_{i,l}^\text{velocity} = \sigma v_{k,\text{L}} + (1-\sigma) v_{k,\text{H}},    
\end{equation}
where $\sigma$ is the weighted scale, which can chosen depending on the position of the target vehicle $k$. If the target is following the host vehicle $i$, then $\sigma$ will ideally be around 0-0.3. If the target is ahead of the host, $\sigma$ will be around 0.7-1.
...
The host vehicle can calculate the relative velocity as 
\begin{equation}
    v_\text{rel} = \dfrac{d(t-1) - d(t)}{t_s},
\end{equation}
 which is derived as the rate of change in distance $d$ over the sample interval $t_s$.

 Then the mismatch score of the acceleration of the target is 
\begin{equation}
   \iota_{i,l}^{\text{accel}} = \max\left(1 - \left|\frac{v_\text{rel}}{d(t)} \cdot max\left(\left|\hat{a}_0^{(l)} - \hat{a}_0^{(i)} \right| - \bar{a}_{th}\right) \right|, 0\right),
\end{equation}
where $\bar{a}_{th}$ is a threshold to ignore small acceleration differences cause by noise, $\left(\hat{a}_0^{(l)} - \hat{a}_0^{(i)}\right)$ is the difference of acceleration between the neighbors of the host and itself.  
The term $\frac{v_\text{rel}}{d(t)}$ scales the effect of acceleration mismatch based on how fast the vehicles are moving relative to each other.
...
\begin{remark}[Expalination on $\Sigma_{\text{local}}$]
...
\begin{equation}
    \T_{i,l}(t) = \gamma_{i,l}^{\text{time}} \cdot  \LT_{i,l} \cdot \DT_{i,l}. 
...
The CACC enhances platoon coordination by leveraging information from multiple preceding vehicles via the distributed observer. For vehicle $i$, the CACC control input is:
\begin{align}
...
\begin{equation}
    a_{\text{target},i} = (1 - \gamma) a_{\text{IDM},i} + \gamma u_{\text{CACC},i},
\end{equation}
where $\gamma \in [0,1]$ is a tuning parameter reflecting trust in communication reliability:
\begin{itemize}
...
To prevent abrupt changes, a first-order filter is applied:
\begin{equation}
    u_i(t) = u_i(t-1) + \tau_f (a_{\text{target},i} - u_i(t-1)),
\end{equation}
where $\tau_f$ is the filter time constant. The vehicle's velocity is then updated as:
\begin{equation}
